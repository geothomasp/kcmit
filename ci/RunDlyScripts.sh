...
if [ "$env" == "kcstage" ]
then
	sudo -u www scp /var/lib/jenkins/jobs/kc_mit_6_0/workspace/kcmit-webapp/target/kcmit-webapp-6.0.0.1.war www@kc-stage-web:/usr/local/tomcat_kc_stage/webapps/kc-stage.war
fi
if [ "$env" == "kcsdbx" ]
then
	sudo -u www scp /var/lib/jenkins/jobs/kc_project_6.0/workspace/coeus-webapp/target/coeus-webapp-6.0.0.war www@kc-stage-web:/usr/local/tomcat_kc_sdbx/webapps/kc-sdbx.war
fi