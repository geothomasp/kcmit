...
EXIT WHEN c_amt_info%NOTFOUND;
  select count(mit_award_number) into li_count from osp$award_amount_transaction where mit_award_number = ls_mit_awd_number;
  IF li_count=1 THEN
...
from award a INNER JOIN REFRESH_AWARD ts on a.AWARD_NUMBER=replace(ts.MIT_AWARD_NUMBER,'-','-00') and a.SEQUENCE_NUMBER=ts.SEQUENCE_NUMBER  
