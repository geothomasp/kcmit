ALTER TRIGGER T_KP_CONFIRM_AFT_INSERT_ROW DISABLE
/
...
COMMIT
/
DELETE FROM AWARD_KEY_PERS_REMOVED WHERE AWARD_NUMBER in (SELECT DISTINCT MIT_AWARD_NUMBER FROM OSP$AWARD_KEY_PERS_REMOVED)
/
commit
/
...
declare
	CURSOR c_award IS
	SELECT replace(AWARD_NUMBER,'-00','-') as MIT_AWARD_NUMBER,SEQUENCE_NUMBER FROM AWARD
	WHERE AWARD_NUMBER in ( SELECT distinct replace(MIT_AWARD_NUMBER,'-','-00') FROM  OSP$AWARD_KEY_PERS_CONFIRM@coeus.kuali );
	...
	exit when c_award%notfound;
		...
		INSERT INTO OSP$AWARD_KEY_PERS_CONFIRM(
			...
			SELECT t0.mit_award_number,
					r_award.sequence_number as sequence_number,
					t0.PERSON_ID,
					t0.CONFIRM_FLAG,
					t0.update_timestamp,
					t0.update_user
					from OSP$AWARD_KEY_PERS_CONFIRM@coeus.kuali t0
					where t0.mit_award_number = r_award.mit_award_number
					and t0.SEQUENCE_NUMBER = (select max(t1.SEQUENCE_NUMBER) from OSP$AWARD_KEY_PERS_CONFIRM@coeus.kuali t1 
											where t1.mit_award_number = t0.MIT_AWARD_NUMBER 
											and t1.sequence_number <= r_award.sequence_number
											and t1.person_id = t0.person_id);				
...
/
delete from AWARD_KEY_PERS_CONFIRM WHERE AWARD_NUMBER in ( select distinct MIT_AWARD_NUMBER from OSP$AWARD_KEY_PERS_CONFIRM)
/
commit
/
set serveroutput on
...
DECLARE
cursor c_data IS
   select distinct t1.award_id, t1.award_number, t1.sequence_number 
   FROM award t1
   inner join OSP$AWARD_KEY_PERS_CONFIRM t2 on t1.award_number = t2.mit_award_number and t1.sequence_number = t2.sequence_number;
...
BEGIN
  ...
  EXIT WHEN c_data%notfound;
    BEGIN
      INSERT INTO AWARD_KEY_PERS_CONFIRM(
       AWARD_KEY_PERS_CONFIRM_ID,
        AWARD_PERSON_ID,
        AWARD_ID,
        AWARD_NUMBER,
        SEQUENCE_NUMBER,
...
COMMIT
/
ALTER TRIGGER T_KP_CONFIRM_AFT_INSERT_ROW ENABLE
/