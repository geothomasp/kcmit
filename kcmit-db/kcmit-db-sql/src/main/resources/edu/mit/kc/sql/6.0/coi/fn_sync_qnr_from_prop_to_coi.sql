...
as_proposal in OSP$QUESTIONNAIRE_ANS_HEADER.MODULE_ITEM_KEY@KC_COI_DB_LINK%type,
as_person_id in OSP$QUESTIONNAIRE_ANS_HEADER.MODULE_SUB_ITEM_KEY@KC_COI_DB_LINK%type,
as_user_id in OSP$USER.USER_ID@KC_COI_DB_LINK%type,
as_actype in char) return number
is
ls_discl_num   NUMBER(10,0);
ls_discl_number	VARCHAR2(10);
ls_sequence_number OSP$COI_DISCLOSURE.SEQUENCE_NUMBER@COEUS.KUALIK%type;
ls_qnr_id OSP$QUESTIONNAIRE.QUESTIONNAIRE_ID@COEUS.KUALIK%type;
...
if as_actype = 'I' then
    begin
		select COI_DISCLOSURE_NUMBER  into ls_discl_number   FROM OSP$COI_DISCLOSURE@COEUS.KUALIK CD
		WHERE PERSON_ID = as_person_id   
		AND ROWNUM = 1;
		
		SELECT max(SEQUENCE_NUMBER) into ls_sequence_number FROM OSP$COI_DISCLOSURE@COEUS.KUALIK WHERE COI_DISCLOSURE_NUMBER = ls_discl_number;
		ls_sequence_number := ls_sequence_number + 1;
		
	exception
	when others then
		select SEQ_COI_DISCLOSURE_NUMBER.nextval@COEUS.KUALIK	into ls_discl_num from dual;
	...
	begin	
        select cd.COI_DISCLOSURE_NUMBER,cd.SEQUENCE_NUMBER INTO ls_discl_number,ls_sequence_number from OSP$COI_DISCLOSURE@COEUS.KUALIK cd
	...
	begin
		  ...
		  FROM OSP$QUESTIONNAIRE@COEUS.KUALIK QNR, OSP$QUESTIONNAIRE_USAGE@COEUS.KUALIK QU
