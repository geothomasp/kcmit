...
FROM OSP$COMM_MEMBERSHIPS@coeus.kuali cm WHERE SEQUENCE_NUMBER=(SELECT MAX(SEQUENCE_NUMBER) FROM OSP$COMM_MEMBERSHIPS@coeus.kuali WHERE COMMITTEE_ID=cm.COMMITTEE_ID and PERSON_ID=cm.PERSON_ID)
AND cm.MEMBERSHIP_ID=(SELECT max(MEMBERSHIP_ID)FROM OSP$COMM_MEMBERSHIPS@coeus.kuali WHERE COMMITTEE_ID=cm.COMMITTEE_ID and PERSON_ID=cm.PERSON_ID 
AND SEQUENCE_NUMBER=(SELECT MAX(SEQUENCE_NUMBER) FROM OSP$COMM_MEMBERSHIPS@coeus.kuali WHERE COMMITTEE_ID=cm.COMMITTEE_ID and PERSON_ID=cm.PERSON_ID));
