...
/
CREATE OR REPLACE package body kc_cac_feed_pkg as
...
BEGIN      
   ...
   close cur_cac_email;
       ...
       -- sending email start
	    ...
	    li_ntfctn_id := KC_MAIL_GENERIC_PKG.FN_INSERT_KREN_NTFCTN('CAC',mail_message);
        begin
            if li_ntfctn_id <>  -1 then 
                KC_MAIL_GENERIC_PKG.FN_INSRT_KREN_NTFCTN_MSG_DELIV(li_ntfctn_id,substr(ls_recipients, 1, INSTR(ls_recipients, '@')-1),ll_MAIL_SENT_STATUS);						
            end if;	
        exception
	    when others then
		NULL;	
...
BEGIN
   ...
   ll_MAIL_SENT_STATUS := 'Y';
        ...
        -- sending email start
		begin
			 KC_MAIL_GENERIC_PKG.GET_NOTIFICATION_TYP_DETS(null,200,'503',mail_subject,mail_message);
			 KC_MAIL_GENERIC_PKG.SEND_MAIL(ls_recipients,null,NULL,mail_subject,mail_message);	
		exception
		when others then
			ll_MAIL_SENT_STATUS := 'N';	
