...
select MIT_AWARD_NUMBER,SEQUENCE_NUMBER,SCIENCE_CODE,UPDATE_TIMESTAMP,UPDATE_USER
from OSP$AWARD_SCIENCE_CODE@coeus.kuali
where MIT_AWARD_NUMBER = as_awd and SEQUENCE_NUMBER = (select max(t.SEQUENCE_NUMBER) from OSP$AWARD_SCIENCE_CODE@coeus.kuali t where t.MIT_AWARD_NUMBER = as_awd and t.SEQUENCE_NUMBER<=as_seq);
...
VALUES(SEQUENCE_AWARD_ID.NEXTVAL,r_refresh.AWARD_ID,1,r_cost.SCIENCE_CODE,r_cost.UPDATE_TIMESTAMP,r_cost.UPDATE_USER,SYS_GUID()); 
