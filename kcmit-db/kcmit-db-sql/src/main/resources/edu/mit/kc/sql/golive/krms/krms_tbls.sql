...
/
DELETE FROM KRMS_AGENDA_ATTR_T WHERE AGENDA_ID IN (SELECT AGENDA_ID FROM KRMS_AGENDA_T WHERE TYP_ID in ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow' ))
/
DELETE FROM KRMS_AGENDA_ITM_T WHERE AGENDA_ID in (SELECT AGENDA_ID FROM KRMS_AGENDA_T WHERE TYP_ID in ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow' ))
/
DELETE FROM KRMS_AGENDA_T WHERE TYP_ID in ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow' )
/
commit
/
DECLARE
...
EXIT WHEN c_data%NOTFOUND;
	...
	SELECT KRMS_AGENDA_ITM_S.nextval,a.RULE_ID,a.SUB_AGENDA_ID,ls_seq,a.VER_NBR,a.WHEN_TRUE,a.WHEN_FALSE,a.ALWAYS 
...
ALTER TABLE KRMS_RULE_T DISABLE CONSTRAINT KRMS_RULE_FK1
/
DELETE FROM KRMS_CMPND_PROP_PROPS_T WHERE PROP_ID in ( SELECT PROP_ID FROM KRMS_PROP_T WHERE TYP_ID in (SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') )
/
DELETE FROM KRMS_PROP_PARM_T WHERE PROP_ID in ( SELECT PROP_ID FROM KRMS_PROP_T WHERE TYP_ID in (SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') )
/
DELETE FROM KRMS_PROP_T WHERE TYP_ID in (SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow')
/
DELETE FROM KRMS_RULE_ATTR_T WHERE RULE_ID in ( SELECT RULE_ID FROM KRMS_RULE_T WHERE PROP_ID IN ( SELECT PROP_ID FROM KRMS_PROP_T WHERE TYP_ID in (SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') ))
/
DELETE FROM KRMS_RULE_T WHERE PROP_ID IN ( SELECT PROP_ID FROM KRMS_PROP_T WHERE TYP_ID in (SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') )
/
DECLARE
	cursor c_data is
	SELECT PROP_ID FROM KRMS_PROP_T@KC_STAG_DB_LINK
	WHERE TYP_ID in (SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow');	
	r_data c_data%rowtype;
...
EXIT WHEN c_data%NOTFOUND;
	...
	SELECT ls_seq,DESC_TXT,TYP_ID,DSCRM_TYP_CD,CMPND_OP_CD,RULE_ID,VER_NBR,CMPND_SEQ_NO FROM KRMS_PROP_T@KC_STAG_DB_LINK
...
EXIT WHEN c_data%NOTFOUND;
	...
	select t1.PROP_ID into ls_prop_id FROM KRMS_PROP_T t1
	inner join KRMS_PROP_T@KC_STAG_DB_LINK t2 on t1.DESC_TXT = t2.DESC_TXT and t1.DSCRM_TYP_CD = t2.DSCRM_TYP_CD 
	and t1.RULE_ID = t2.RULE_ID;	
	
	INSERT INTO KRMS_RULE_T(RULE_ID,NMSPC_CD,NM,TYP_ID,PROP_ID,ACTV,VER_NBR,DESC_TXT)
	SELECT ls_seq,NMSPC_CD,NM,TYP_ID,ls_prop_id,ACTV,VER_NBR,DESC_TXT FROM KRMS_RULE_T@KC_STAG_DB_LINK
