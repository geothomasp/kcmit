...
WHERE b.ACTV = 'Y'
AND a.ACTN_ID in ( select actn_id from krms_actn_t where typ_id in (SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') )
/
...
KRMS_ATTR_DEFN_T@KC_STAG_DB_LINK b on a.ATTR_DEFN_ID=b.ATTR_DEFN_ID
where b.ACTV='Y'
AND   a.AGENDA_ID IN (SELECT AGENDA_ID FROM KRMS_AGENDA_T WHERE TYP_ID in ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow' ))
/
...
where b.ACTV='Y'
AND b.TYP_ID in ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow' )
/
...
WHERE TYP_ID in ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow' )
/
...
WHERE PROP_ID in ( SELECT PROP_ID FROM KRMS_PROP_T WHERE TYP_ID in (SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') )
/
...
WHERE b.ACTV= 'Y'
AND b.PROP_ID IN ( SELECT PROP_ID FROM KRMS_PROP_T WHERE TYP_ID in (SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') )
/
ALTER TABLE KRMS_RULE_T DISABLE CONSTRAINT KRMS_RULE_FK1
/
...
begin
  ...
  select max(ltrim(PROP_ID,'KC')) into ls_max_val from KRMS_PROP_T;
  if ls_max_val is null then
  ls_max_val := 0;
  end if;    
...
begin
  ...
  select max(ltrim(RULE_ATTR_ID,'KC')) into ls_max_val from KRMS_RULE_ATTR_T;
  if ls_max_val is null then
  ls_max_val := 0;
  end if;    
...
begin
  ...
  select max(ltrim(RULE_ID,'KC')) into ls_max_val from KRMS_RULE_T;
  if ls_max_val is null then
  ls_max_val := 0;
  end if;    
...
begin
  ...
  select max(ltrim(TERM_PARM_ID,'KC')) into ls_max_val from KRMS_TERM_PARM_T;
  if ls_max_val is null then
  ls_max_val := 0;
  end if;    
...
begin
  ...
  select max(ltrim(TERM_RSLVR_PARM_SPEC_ID,'KC')) into ls_max_val from KRMS_TERM_RSLVR_PARM_SPEC_T;
  if ls_max_val is null then
  ls_max_val := 0;
  end if;    
...
begin
  ...
  select max(ltrim(TERM_RSLVR_ID,'KC')) into ls_max_val from KRMS_TERM_RSLVR_T;
  if ls_max_val is null then
  ls_max_val := 0;
  end if;    
...
begin
  ...
  select max(ltrim(TERM_ID,'KC')) into ls_max_val from KRMS_TERM_T;
  if ls_max_val is null then
  ls_max_val := 0;
  end if;    
...
begin
  ...
  select max(ltrim(TERM_SPEC_ID,'KC')) into ls_max_val from KRMS_TERM_SPEC_T;
  if ls_max_val is null then
  ls_max_val := 0;
  end if;    
...
begin
  ...
  select max(ltrim(TYP_ATTR_ID,'KC')) into ls_max_val from KRMS_TYP_ATTR_T;
  if ls_max_val is null then
  ls_max_val := 0;
  end if;    
...
begin
  ...
  select max(ltrim(TYP_ID,'KC')) into ls_max_val from KRMS_TYP_T;
  if ls_max_val is null then
  ls_max_val := 0;
  end if;    
