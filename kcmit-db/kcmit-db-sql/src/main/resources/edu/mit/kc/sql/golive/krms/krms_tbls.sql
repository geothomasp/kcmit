...
/
---------------------------
DELETE FROM KRMS_CMPND_PROP_PROPS_T WHERE cmpnd_prop_id in (  SELECT PROP_ID FROM KRMS_PROP_T WHERE RULE_ID in ( SELECT RULE_ID FROM KRMS_ACTN_T WHERE typ_id IN ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') ) )
/
DELETE FROM KRMS_PROP_PARM_T WHERE PROP_ID in ( SELECT PROP_ID FROM KRMS_PROP_T WHERE RULE_ID in ( SELECT RULE_ID FROM KRMS_ACTN_T WHERE typ_id IN ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') ) )
/
DELETE FROM KRMS_PROP_T WHERE RULE_ID in ( SELECT RULE_ID FROM KRMS_ACTN_T WHERE typ_id IN ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') )
/
DELETE FROM KRMS_RULE_ATTR_T WHERE RULE_ID in ( SELECT RULE_ID FROM KRMS_ACTN_T WHERE typ_id IN ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') )
/
DELETE FROM KRMS_RULE_T WHERE RULE_ID IN ( SELECT RULE_ID FROM KRMS_ACTN_T WHERE typ_id IN ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') )
/
drop table TEMP_MAPPING_KRMS_RULE_T
/
CREATE TABLE TEMP_MAPPING_KRMS_RULE_T(
RULE_ID VARCHAR2(40),
...
DECLARE
	...
	ls_prop_id KRMS_PROP_T.PROP_ID%type;
BEGIN
OPEN c_data;
LOOP
FETCH c_data INTO r_data;
EXIT WHEN c_data%NOTFOUND;
	ls_seq := KRMS_RULE_S.nextval;
	
	INSERT INTO KRMS_RULE_T(RULE_ID,NMSPC_CD,NM,TYP_ID,PROP_ID,ACTV,VER_NBR,DESC_TXT)
	SELECT ls_seq,NMSPC_CD,NM,TYP_ID,NULL,ACTV,VER_NBR,DESC_TXT FROM KRMS_RULE_T@KC_STAG_DB_LINK
	WHERE RULE_ID = r_data.RULE_ID;
	
	INSERT INTO KRMS_RULE_ATTR_T(RULE_ATTR_ID,RULE_ID,ATTR_DEFN_ID,ATTR_VAL,VER_NBR)
	SELECT  KRMS_RULE_ATTR_S.nextval,ls_seq,a.ATTR_DEFN_ID,a.ATTR_VAL,a.VER_NBR FROM KRMS_RULE_ATTR_T@KC_STAG_DB_LINK a
	INNER JOIN KRMS_RULE_T@KC_STAG_DB_LINK b
	ON a.RULE_ID = b.RULE_ID
	WHERE b.RULE_ID = r_data.RULE_ID;
	
	INSERT INTO TEMP_MAPPING_KRMS_RULE_T(RULE_ID,NEW_RULE_ID)
	VALUES(r_data.RULE_ID,ls_seq);
...
/
------------------
CREATE TABLE TEMP_MAPPING_KRMS_PROP_T(
PROP_ID VARCHAR2(40),
...
FETCH c_data INTO r_data;
EXIT WHEN c_data%NOTFOUND;
	ls_seq := KRMS_PROP_S.nextval;	
	
	INSERT INTO KRMS_PROP_T(PROP_ID,DESC_TXT,TYP_ID,DSCRM_TYP_CD,CMPND_OP_CD,RULE_ID,VER_NBR,CMPND_SEQ_NO)
	SELECT ls_seq,DESC_TXT,TYP_ID,DSCRM_TYP_CD,CMPND_OP_CD,
	(select NEW_RULE_ID from temp_mapping_krms_rule_t where rule_id = r_data.rule_id ),
	VER_NBR,CMPND_SEQ_NO FROM KRMS_PROP_T@KC_STAG_DB_LINK
	WHERE PROP_ID = r_data.PROP_ID;

	INSERT INTO KRMS_CMPND_PROP_PROPS_T(CMPND_PROP_ID,PROP_ID)
	SELECT CMPND_PROP_ID,ls_seq FROM KRMS_CMPND_PROP_PROPS_T@KC_STAG_DB_LINK
	WHERE PROP_ID = r_data.PROP_ID;
	
	INSERT INTO TEMP_MAPPING_KRMS_PROP_T(PROP_ID,NEW_PROP_ID)
	VALUES(r_data.PROP_ID,ls_seq);
		
	INSERT INTO KRMS_PROP_PARM_T(PROP_PARM_ID,PROP_ID,PARM_VAL,PARM_TYP_CD,SEQ_NO,VER_NBR)
	SELECT KRMS_PROP_PARM_S.nextval,ls_seq,PARM_VAL,PARM_TYP_CD,SEQ_NO,VER_NBR FROM KRMS_PROP_PARM_T@KC_STAG_DB_LINK
	WHERE PROP_ID = r_data.PROP_ID;
	
	UPDATE KRMS_RULE_T SET PROP_ID = ls_seq WHERE RULE_ID = (select NEW_RULE_ID from temp_mapping_krms_rule_t where rule_id = r_data.rule_id );
...
commit
/
UPDATE krms_cmpnd_prop_props_t t1 set t1.cmpnd_prop_id = (select distinct t2.new_prop_id from temp_mapping_krms_prop_t t2 where t2.prop_id = t1.cmpnd_prop_id )
where t1.cmpnd_prop_id in ( select prop_id from temp_mapping_krms_prop_t )
/
drop table TEMP_MAPPING_KRMS_PROP_T
/
DELETE FROM KRMS_AGENDA_ATTR_T WHERE AGENDA_ID IN (SELECT AGENDA_ID FROM KRMS_AGENDA_ITM_T WHERE RULE_ID IN ( SELECT RULE_ID FROM KRMS_ACTN_T WHERE typ_id IN ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') ))
/
DELETE FROM KRMS_AGENDA_T WHERE AGENDA_ID IN (SELECT AGENDA_ID FROM KRMS_AGENDA_ITM_T WHERE RULE_ID IN ( SELECT RULE_ID FROM KRMS_ACTN_T WHERE typ_id IN ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') ))
/
DELETE FROM KRMS_AGENDA_ITM_T WHERE RULE_ID IN ( SELECT RULE_ID FROM KRMS_ACTN_T WHERE typ_id IN ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') )
/
DECLARE
	cursor c_data is
...
EXIT WHEN c_data%NOTFOUND;
	...
	ls_seq := KRMS_AGENDA_S.nextval;	
	
	INSERT INTO KRMS_AGENDA_T(AGENDA_ID,NM,CNTXT_ID,INIT_AGENDA_ITM_ID,TYP_ID,ACTV,VER_NBR)
	SELECT ls_seq,NM,CNTXT_ID,INIT_AGENDA_ITM_ID,TYP_ID,ACTV,VER_NBR FROM KRMS_AGENDA_T@KC_STAG_DB_LINK
	WHERE AGENDA_ID = r_data.AGENDA_ID;
		
	INSERT INTO KRMS_AGENDA_ATTR_T(AGENDA_ATTR_ID,AGENDA_ID,ATTR_VAL,ATTR_DEFN_ID,VER_NBR)
	SELECT KRMS_AGENDA_ATTR_S.nextval,ls_seq,a.ATTR_VAL,a.ATTR_DEFN_ID,a.VER_NBR FROM KRMS_AGENDA_ATTR_T@KC_STAG_DB_LINK a INNER JOIN 
	KRMS_ATTR_DEFN_T@KC_STAG_DB_LINK b on a.ATTR_DEFN_ID=b.ATTR_DEFN_ID
	where b.ACTV='Y'
	AND   a.AGENDA_ID = r_data.AGENDA_ID;

	INSERT INTO KRMS_AGENDA_ITM_T(AGENDA_ITM_ID,RULE_ID,SUB_AGENDA_ID,AGENDA_ID,VER_NBR,WHEN_TRUE,WHEN_FALSE,ALWAYS)
	SELECT KRMS_AGENDA_ITM_S.nextval,	
	(select NEW_RULE_ID from temp_mapping_krms_rule_t where rule_id = r_data.rule_id ),
	a.SUB_AGENDA_ID,ls_seq,a.VER_NBR,a.WHEN_TRUE,a.WHEN_FALSE,a.ALWAYS 
	FROM KRMS_AGENDA_ITM_T@KC_STAG_DB_LINK a
	INNER JOIN KRMS_AGENDA_T@KC_STAG_DB_LINK b on a.AGENDA_ID = b.AGENDA_ID
	where b.AGENDA_ID = r_data.AGENDA_ID;
...
/
DELETE FROM KRMS_ACTN_ATTR_T WHERE ACTN_ID in ( select actn_id from krms_actn_t where RULE_ID in ( SELECT RULE_ID FROM KRMS_ACTN_T WHERE typ_id IN ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') ) )
/
DELETE FROM KRMS_ACTN_T WHERE RULE_ID in ( SELECT RULE_ID FROM KRMS_ACTN_T WHERE typ_id IN ( SELECT typ_id FROM krms_typ_t WHERE nm = 'Route to PeopleFlow') )
/
DECLARE
	cursor c_data is
...
EXIT WHEN c_data%NOTFOUND;
	...
	SELECT ls_seq,a.NM,a.DESC_TXT,a.TYP_ID,(select NEW_RULE_ID from temp_mapping_krms_rule_t where rule_id = r_data.rule_id ),
