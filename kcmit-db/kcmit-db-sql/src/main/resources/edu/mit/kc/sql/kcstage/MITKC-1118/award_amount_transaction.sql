DELETE FROM AWARD_AMOUNT_INFO WHERE award_id in (select award_id from REFRESH_AWARD_BAK)
/
commit
/
DECLARE
...
INNER JOIN REFRESH_AWARD_BAK ts on a.AWARD_NUMBER=replace(ts.MIT_AWARD_NUMBER,'-','-00') and a.SEQUENCE_NUMBER=ts.SEQUENCE_NUMBER 
...
EXIT WHEN c_amt_info%NOTFOUND;
  select count(mit_award_number) into li_count from osp$award_amount_transaction@coeus.kuali where mit_award_number = ls_mit_awd_number;
  IF li_count > 0 THEN
...
FROM AWARD_AMOUNT_INFO a INNER JOIN REFRESH_AWARD_BAK ts on a.AWARD_NUMBER=replace(ts.MIT_AWARD_NUMBER,'-','-00') and a.SEQUENCE_NUMBER=ts.SEQUENCE_NUMBER
