select 'Started UPDATE_AWARD_IDC_RATE ' from dual
/
DECLARE
...
exception
when others then
	dbms_output.put_line('Error in update of AWARD_IDC_RATE. AWARD_NUMBER,SEQUENCE_NUMBER'||r_award_comment.AWARD_NUMBER||','||r_award_comment.SEQUENCE_NUMBER||'-'||sqlerrm);
end;	
...
END;
/
select 'Ended UPDATE_AWARD_IDC_RATE ' from dual
/