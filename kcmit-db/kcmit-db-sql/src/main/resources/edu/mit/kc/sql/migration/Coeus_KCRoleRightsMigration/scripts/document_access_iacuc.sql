...
li_count number;
begin
  select  count(table_name) into li_count  from user_tables   where table_name = 'OSP$AC_PROTOCOL_USER_ROLES';
  if li_count = 0 then
    execute immediate('	CREATE TABLE OSP$AC_PROTOCOL_USER_ROLES(
...
begin
   ...
   select count(*) into li_count from OSP$AC_PROTOCOL_USER_ROLES;
   if li_count = 0 then    
      INSERT INTO OSP$AC_PROTOCOL_USER_ROLES(
      PROTOCOL_NUMBER,
	  SEQUENCE_NUMBER,
      ROLE_ID,
      USER_ID,
      ...
      UPDATE_TIMESTAMP,
      UPDATE_USER
      FROM OSP$AC_PROTOCOL_USER_ROLES@coeus.kuali;         
...
CURSOR c_module_role is
	...
	FROM OSP$AC_PROTOCOL_USER_ROLES e 
	inner join IACUC_PROTOCOL t1 on t1.protocol_number = e.protocol_number and t1.sequence_number = e.sequence_number
