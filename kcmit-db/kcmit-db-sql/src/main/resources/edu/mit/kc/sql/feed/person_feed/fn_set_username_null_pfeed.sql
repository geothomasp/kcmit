...
to NULL durting the feed process.
Once Emails are generated, this function does the actual updates
to set user_names to NULL for those persons who are in osp$person table
...
BEGIN
	...
	loop
		...
		BEGIN
			   ...
			   mail_message:='User_names of following Prncpl_IDs were set as NULL in krim_prncpl_t table:'|| ls_PersonId ||tab || ls_UserName ||tab || ls_FullName || crlf;			
			   li_mail_send := 'Y';
				begin
...
/*
	...
	WHEN OTHERS THEN
		return SUBSTR('Error Sending mail to  ' || as_EmailId || ': ' || SQLERRM, 1, 200);
*/
END;
/
