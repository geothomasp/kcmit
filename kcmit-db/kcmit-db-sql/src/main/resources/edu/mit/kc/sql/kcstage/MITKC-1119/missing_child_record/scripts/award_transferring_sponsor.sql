...
select MIT_AWARD_NUMBER,SEQUENCE_NUMBER,SPONSOR_CODE,UPDATE_TIMESTAMP,UPDATE_USER
from OSP$AWARD_TRANSFERRING_SPONSOR@coeus.kuali 
where MIT_AWARD_NUMBER = as_awd and SEQUENCE_NUMBER = (select max(t.SEQUENCE_NUMBER) from OSP$AWARD_TRANSFERRING_SPONSOR@coeus.kuali  t where t.MIT_AWARD_NUMBER = as_awd and t.SEQUENCE_NUMBER<=as_seq);
...
VALUES(SEQ_AWARD_TRANS_SPONSOR_ID.NEXTVAL,r_refresh.AWARD_ID,ls_awd_num,r_refresh.sequence_number,r_cost.SPONSOR_CODE,r_cost.UPDATE_TIMESTAMP,r_cost.UPDATE_USER,1,SYS_GUID());  
