select ' Start time of AWARD_PERSONS(Investigators) script is '|| localtimestamp from dual
/
DECLARE
...
LEFT OUTER JOIN OSP$AWARD_INVESTIGATORS@coeus.kuali ac ON t.MIT_AWARD_NUMBER=ac.MIT_AWARD_NUMBER and t.SEQUENCE_NUMBER=ac.SEQUENCE_NUMBER
...
EXIT WHEN c_award_comment%NOTFOUND;
    ...
    IF r_award_comment.MIT_AWARD_NUMBER IS NULL THEN
	   ...
	   IF ls_award_number is null THEN
	        ...
	        SELECT SEQUENCE_AWARD_ID.NEXTVAL,KEY_PERSON_PROJECT_ROLE,r_award_comment.AWARD_ID,r_award_comment.AWARD_NUMBER,r_award_comment.Kuali_sequence_number,PERSON_ID,ROLODEX_ID,FULL_NAME,CONTACT_ROLE_CODE,ACADEMIC_YEAR_EFFORT,CALENDAR_YEAR_EFFORT,SUMMER_EFFORT,TOTAL_EFFORT,FACULTY_FLAG,UPDATE_TIMESTAMP,UPDATE_USER,1,SYS_GUID(),MULTIPLE_PI FROM AWARD_PERSONS
            WHERE AWARD_NUMBER=r_award_comment.AWARD_NUMBER AND SEQUENCE_NUMBER=(SELECT MAX(aw.SEQUENCE_NUMBER) FROM AWARD_PERSONS aw WHERE aw.AWARD_NUMBER=r_award_comment.AWARD_NUMBER AND SEQUENCE_NUMBER<r_award_comment.Kuali_sequence_number);
...
select ' End time of AWARD_PERSONS(Investigators) script is '|| localtimestamp from dual
/