...
SELECT a.AWARD_NUMBER,a.SEQUENCE_NUMBER  Kuali_sequence_number,a.AWARD_ID,ac.MIT_AWARD_NUMBER,ac.SEQUENCE_NUMBER,DECODE(r.ROLODEX_ID,null,ac.PERSON_ID,null) PERSON_ID,r.ROLODEX_ID,decode(ac.PRINCIPAL_INVESTIGATOR_FLAG,'Y','PI','N','COI') ROLE_CODE,ac.PERSON_NAME,ac.PRINCIPAL_INVESTIGATOR_FLAG,ac.NON_MIT_PERSON_FLAG,NVL(ac.FACULTY_FLAG,'N') FACULTY_FLAG,ac.CONFLICT_OF_INTEREST_FLAG,ac.PERCENTAGE_EFFORT,ac.FEDR_DEBR_FLAG,ac.FEDR_DELQ_FLAG,ac.UPDATE_TIMESTAMP,ac.UPDATE_USER,ac.MULTI_PI_FLAG,ac.ACADEMIC_YEAR_EFFORT,ac.SUMMER_YEAR_EFFORT,ac.CALENDAR_YEAR_EFFORT FROM AWARD a 
...
EXIT WHEN c_award_comment%NOTFOUND;
    ...
    IF r_award_comment.MIT_AWARD_NUMBER IS NULL THEN
	   ...
	   IF ls_award_number is null THEN
            ...
            WHERE AWARD_NUMBER=r_award_comment.AWARD_NUMBER AND SEQUENCE_NUMBER=(SELECT MAX(aw.SEQUENCE_NUMBER) FROM AWARD_PERSONS aw WHERE aw.AWARD_NUMBER=r_award_comment.AWARD_NUMBER AND SEQUENCE_NUMBER<li_seq);
