...
<c:set var="awardPersonAttributes"
	value="${DataDictionary.AwardPerson.attributes}" />
<c:set var="keyPersonRoleConstant"
	value="<%=org.kuali.kra.infrastructure.Constants.KEY_PERSON_ROLE%>" />
	...
	htmlFormAction="keyperson">
		...
		<div class="tab-container" align="center">
			<h3>
				...
				<kul:innerTab tabTitle="Key Personnel"
				parentTab="Key Personnel and Credit Split" defaultOpen="true"
				...
				auditCluster="contactsAuditErrors,contactsAuditWarnings"
				tabAuditKey="document.awardList[0].projectPerson*">
				<table id="contacts-table" cellpadding="0" cellspacing="0"
					summary="Key Personnel">
					<tr>
					...
					<c:if test="${!readOnly}">
							...
							<tr>
								...
								<td nowrap class="grid" class="infoline">
									<table style="border: none;">
										<tr>
											<td style="white-space: nowrap; border: none;">Employee
												User Name:</td>
												
										 	<td style="white-space: nowrap; border: none;"><%-- <kul:htmlControlAttribute
													property="projectPersonnelBean.newProjectPerson.person.fullName"
													...
													<kul:lookup
														...
														fieldConversions="personId:projectPersonnelBean.personId"
														anchor="${tabKey}"
														lookupParameters="projectPersonnelBean.newProjectPerson.person.fullName:lastName" />

												</c:if> <c:if test="${readOnly}">
													<html:hidden styleId="per.fullName.div"
														property="projectPersonnelBean.newProjectPerson.person.fullName" />
												</c:if> <html:hidden styleId="key.personId"
													property="projectPersonnelBean.personId" /> --%>
												${kfunc:registerEditableProperty(KualiForm,
												"projectPersonnelBean.personId")}

												<div id="per.fullName.div">
													&nbsp;
													<c:if
												...
												<tr>
											<td style="white-space: nowrap; border: none;"><label><span
													style="margin-right: 3;">Non-employee ID: </td>
													 <td style="white-space: nowrap; border: none;"><%-- <kul:htmlControlAttribute
													property="projectPersonnelBean.newProjectPerson.rolodex.fullName"
													attributeEntry="${awardPersonAttributes.fullName}"
													...
													readOnly="${readOnly}" /> <c:if test="${!readOnly}">
													<kul:lookup
														boClassName="org.kuali.coeus.common.framework.rolodex.NonOrganizationalRolodex"
														fieldConversions="rolodexId:projectPersonnelBean.rolodexId"
														anchor="${tabKey}"
													...
													<html:hidden styleId="rol.fullName.div"
														property="projectPersonnelBean.newProjectPerson.rolodex.fullName" />
												</c:if> ${kfunc:registerEditableProperty(KualiForm,
												"projectPersonnelBean.rolodexId")} <html:hidden
													styleId="key.rolodexId"
													property="projectPersonnelBean.rolodexId" /> --%>

												<div id="rol.fullName.div">
													&nbsp;
													<c:if
								...
								<div align="center">
										<%-- <kul:htmlControlAttribute
											property="projectPersonnelBean.contactRoleCode"
											attributeEntry="${awardPersonAttributes.contactRoleCode}"
											onchange="proposalRoleChange(this, 400);" />
										<br /> <span class="keypersononly">
											<div class="ignoreMeFromWarningOnAddRow">
												*
												<kul:htmlAttributeLabel
													attributeEntry="${awardPersonAttributes.keyPersonRole}"
													useShortLabel="true" noColon="false" />
											</div> <span class="noscriptonly">(Required for Key Persons
												only)</span>
											<div class="ignoreMeFromWarningOnAddRow">
												<kul:htmlControlAttribute
													property="projectPersonnelBean.newAwardContact.keyPersonRole"
													attributeEntry="${awardPersonAttributes.keyPersonRole}" />
											</div>
										</span> --%>
                          ...
                          $jq(document).ready(function() {
                              ...
                              $jq(document).find("[id$='contactRoleCode']").each(function() {
								...
								<td id="key.phoneNumber" class="infoline">
									<div align="center">
										<c:out
											value="${KualiForm.projectPersonnelBean.newAwardContact.contact.phoneNumber}" />
										&nbsp;
									</div>
								</td>
								<td id="key.emailAddress" class="infoline">
									<div align="center">
										<c:out
											value="${KualiForm.projectPersonnelBean.newAwardContact.contact.emailAddress}" />
										&nbsp;
									</div>
								</td>
								<td class="infoline">Sync Mode:&nbsp;${KualiForm.syncMode}</td>
								<td class="infoline">
									<div align="center">
										<html:image property="methodToCall.addProjectPerson"
											src="${ConfigProperties.kr.externalizable.images.url}tinybutton-add1.gif"
											title="Add Contact" alt="Add Contact"
											styleClass="tinybutton addButton" />
												...
												<tr>
						<td colspan="7">
							KP Role:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${KualiForm.kpMaintenanceRole}</td>
					</tr>
					
						<c:forEach var="awardContact"
						items="${KualiForm.document.awardList[0].projectPersons}"
						varStatus="awardContactRowStatus">
						<tr>
							<th class="infoline" scope="row" rowspan="4"><c:out
									value="${awardContactRowStatus.index + 1}" /></th>
							<td valign="middle"><input type="hidden"
								name="award_person.identifier_${awardContactRowStatus.index}"
								...
								<div align="center">
									...
									<c:choose>
										<c:when test="${awardContact.employee}">
											<kul:directInquiry
												boClassName="org.kuali.coeus.common.framework.person.KcPerson"
												inquiryParameters="award_person.identifier_${awardContactRowStatus.index}:personId"
												anchor="${tabKey}" />
										</c:when>
										<c:otherwise>
											<kul:directInquiry
												boClassName="org.kuali.coeus.common.framework.rolodex.NonOrganizationalRolodex"
												inquiryParameters="award_person.identifier_${awardContactRowStatus.index}:rolodexId"
												anchor="${tabKey}" />
										</c:otherwise>
									</c:choose> --%>
								</div></td>
							<td valign="middle">
								<div align="center">
									<%-- <input type="hidden"
										name="award_person.orgNumber_${awardContactRowStatus.index}"
									...
									<c:choose>
										<c:when test="${awardContact.employee}">
											<kul:directInquiry
												...
												inquiryParameters="award_person.orgNumber_${awardContactRowStatus.index}:unitNumber"
												anchor="${tabKey}" />
										</c:when>
										<c:otherwise>
											<kul:directInquiry
							...
							<td valign="middle">
								...
								<div align="center">
									<%-- <kul:htmlControlAttribute
										property="projectPersonnelBean.projectPersonnel[${awardContactRowStatus.index}].contactRoleCode"
										attributeEntry="${awardPersonAttributes.contactRoleCode}"
										onchange="proposalRoleChange(this, 400);" />
									<br /> <span class="keypersononly"> *<kul:htmlAttributeLabel
											attributeEntry="${awardPersonAttributes.keyPersonRole}"
											useShortLabel="true" noColon="false" /> <span
										class="noscriptonly">(Required for Key Persons only)</span> <kul:htmlControlAttribute
											property="projectPersonnelBean.projectPersonnel[${awardContactRowStatus.index}].keyPersonRole"
											attributeEntry="${awardPersonAttributes.keyPersonRole}" />
									</span> --%>
								</div> ${KualiForm.valueFinderResultCache}
								...
								<div align="center">
									...
									<c:if test="${!readOnly}">
										<c:if test="${KualiForm.syncMode}">
											<html:image
												property="methodToCall.syncProjectPerson.line${awardContactRowStatus.index}.anchor${currentTabIndex}"
												src='${ConfigProperties.kra.externalizable.images.url}tinybutton-sync.gif'
												alt="sync" styleClass="tinybutton" disabled="${readOnly}" />
										</c:if>
									</c:if>
						...
						<tr>
							<td colspan="7"><%-- <kra-a:awardProjectPersonnelPersonDetails
									awardContact="${awardContact}"
									awardContactRowStatusIndex="${awardContactRowStatus.index}" /> --%>
							</td>
						</tr>
						<tr>
							<td colspan="7"><%-- <kra-a:awardProjectPersonnelUnits
									awardContact="${awardContact}"
									awardPersonIndex="${awardContactRowStatus.index}" /> --%></td>
						</tr>
