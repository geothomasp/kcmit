 ...
 */
public class MitKcUnitTestRunner extends BlockJUnit4ClassRunner {
     ...
     */
    public MitKcUnitTestRunner(Class<?> klass) throws InitializationError {
        super(klass);
    }
    ...
    protected Statement methodInvoker(FrameworkMethod method, Object test) {
        if (test instanceof MitKcUnitTestMethodAware) {
            ((MitKcUnitTestMethodAware) test).setTestMethod(method.getMethod());
        }
    ...
    public void run(RunNotifier notifier) {
        if (!listenerAdded) {
            notifier.addListener(MitKcUnitTestBase.getRunListener());
            listenerAdded = true;
        }
        super.run(notifier);
    }
