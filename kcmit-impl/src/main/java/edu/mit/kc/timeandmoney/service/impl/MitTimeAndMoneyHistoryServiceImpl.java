package edu.mit.kc.timeandmoney.service.impl;
...
import edu.mit.kc.timeandmoney.service.MitTimeAndMoneyHistoryService;

public class MitTimeAndMoneyHistoryServiceImpl implements MitTimeAndMoneyHistoryService{
  ...
  @SuppressWarnings("unchecked")
            ...
            if(!(awardAmountInfo.getTimeAndMoneyDocumentNumber() == null)) {
                    if(StringUtils.equalsIgnoreCase(doc.getDocumentNumber(),awardAmountInfo.getTimeAndMoneyDocumentNumber())){ 
                        if(awardAmountInfo.getTransactionId() == null) {
                            ...
                            List<AwardAmountInfo> dateTransactionDetails = ((List<AwardAmountInfo>)businessObjectService.findMatching(AwardAmountInfo.class, fieldValues));
    ...
    protected String getUpdateTimeAndUser(TimeAndMoneyDocument doc) {
        ...
        if (doc.getUpdateTimestamp() != null) {
            createDateStr = CoreApiServiceLocator.getDateTimeService().toString(doc.getUpdateTimestamp(), "MM/dd/yy hh:mm:ss a");
