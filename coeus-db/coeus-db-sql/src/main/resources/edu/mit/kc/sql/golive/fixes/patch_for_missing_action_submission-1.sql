...
li_cnt number;
	cursor c_protocol_submission is
	select distinct pa.proto_amend_ren_number,pa.protocol_number,pa.sequence_number,ps.submission_number,ps.action_id
...
exit when c_protocol_submission%notfound;
      ...
      select count(protocol_number) into li_cnt from protocol_correspondence 
	  where protocol_number = r_protocol_submission.proto_amend_ren_number 
	  and sequence_number = 0 
