...
/
DELETE FROM TMP_MISSING_AMEND_RENW_PROTO
/
...
exit when c_missing%notfound;
	...
	if li_count = 0 then
	   ...
	   continue;
					...
					exception
                    when others then
						dbms_output.put_line('Exception in protocol_id 123 ,for PROTO_AMEND_REN_NUMBER '||r_missing.proto_amend_ren_number||', Error is '||sqlerrm);
                    end;
