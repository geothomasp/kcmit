select ' Started VERSION_HISTORY  ' from dual
/
declare
...
decode((select max(a.sequence_number) from award a  where a.award_number = awd.award_number),awd.sequence_number,'ACTIVE','ARCHIVED') version_status ,
 ...
 from award awd INNER JOIN TEMP_TAB_TO_SYNC_AWARD t ON awd.AWARD_NUMBER = replace(t.MIT_AWARD_NUMBER,'-','-00') and awd.SEQUENCE_NUMBER=t.SEQUENCE_NUMBER  
where t.FEED_TYPE='N'  ;
commit;
...
END;
/
select ' Ended VERSION_HISTORY  ' from dual
/