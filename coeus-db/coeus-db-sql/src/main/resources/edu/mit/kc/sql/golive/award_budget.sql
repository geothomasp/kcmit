...
begin
	...
	exit when c_data%notfound;
		...
		begin
			select budget_justification into ll_bgt_justi_clob from TMP_STAG_BUDGET  where BUDGET_ID = r_data.BUDGET_ID; 			
		exception when others then
			ll_bgt_justi_clob:=null;
		end;
		
		begin
			select COMMENTS into ll_comment from TMP_STAG_BUDGET  where BUDGET_ID = r_data.BUDGET_ID; 			
		exception when others then
			ll_comment:=null;
		end;
	...
	VALUES(r_data.prod_budget_id,ll_bgt_justi_clob ,r_data.on_off_campus_flag,r_data.version_number,r_data.prod_bud_doc_id,r_data.parent_document_type_code,
	...
	r_data.total_cost_limit,r_data.oh_rate_class_code,r_data.oh_rate_type_code,ll_comment,r_data.final_version_flag,r_data.update_timestamp,r_data.update_user,r_data.ur_rate_class_code,
...
exit when c_data%notfound;
    ...
    begin
			select budget_justification into ll_bgt_justi_clob from TMP_STAG_BUD_PER_DET 
      where budget_personnel_details_id = r_data.budget_personnel_details_id; 			
		exception when others then
			ll_bgt_justi_clob:=null;
		end;
    ...
    r_data.on_off_campus_flag,r_data.apply_in_rate_flag,ll_bgt_justi_clob,
