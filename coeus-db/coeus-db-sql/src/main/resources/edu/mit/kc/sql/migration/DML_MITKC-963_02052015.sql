...
inner join OSP$PROPOSAL@coeus.kuali ad on p.PROPOSAL_NUMBER=ad.PROPOSAL_NUMBER and p.SEQUENCE_NUMBER=ad.SEQUENCE_NUMBER left outer join ROLODEX r on a.PERSON_ID=r.ROLODEX_ID 
...
SEQUENCE_NUMBER=r_kp_unit_main.SEQUENCE_NUMBER AND(PERSON_ID=r_pers_units.PERSON_ID OR ROLODEX_ID=r_pers_units.PERSON_ID) AND contact_role_code IN ('PI','COI');

ELSE
SELECT PROPOSAL_PERSON_ID INTO li_prop_pers_id FROM PROPOSAL_PERSONS WHERE PROPOSAL_NUMBER=r_pers_units.PROPOSAL_NUMBER AND
SEQUENCE_NUMBER=r_kp_unit_main.SEQUENCE_NUMBER AND(PERSON_ID=r_pers_units.PERSON_ID OR ROLODEX_ID=r_pers_units.PERSON_ID)  AND contact_role_code IN ('KP');
