...
/
DROP TABLE ORGANIZATION_MAPPING
/
...
CURSOR c_AWRD IS  SELECT to_number(PROPOSAL_NUMBER) PROPOSAL_NUMBER, VERSION_NUMBER, SUB_AWARD_NUMBER,ORGANIZATION_NAME, SUB_AWARD_STATUS_CODE , COMMENTS, XFD_UPDATE_USER, XFD_UPDATE_TIMESTAMP,  TRANSLATION_COMMENTS, XML_UPDATE_USER, XML_UPDATE_TIMESTAMP , UPDATE_TIMESTAMP, UPDATE_USER, NAMESPACE, FORM_NAME, nvl(SUB_AWARD_XFD_FILE_NAME,'dummy') SUB_AWARD_XFD_FILE_NAME , nvl(SUB_AWARD_XFD_FILE,EMPTY_BLOB()) SUB_AWARD_XFD_FILE,SUB_AWARD_XML_FILE FROM TEMP_BUD_SUB_AWD;
r_AWRD c_AWRD%ROWTYPE;

CURSOR c_sub_awd_att is
SELECT  to_number(PROPOSAL_NUMBER) PROPOSAL_NUMBER,VERSION_NUMBER,SUB_AWARD_NUMBER,CONTENT_ID,CONTENT_TYPE,ATTACHMENT,UPDATE_TIMESTAMP,UPDATE_USER  FROM TEMP_BUD_SUB_AWD_ATT;
