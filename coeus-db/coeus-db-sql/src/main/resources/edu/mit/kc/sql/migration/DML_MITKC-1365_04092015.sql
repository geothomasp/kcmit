...
SELECT FILE_ID FROM SUBAWARD_AMOUNT_INFO WHERE SUBAWARD_CODE in ( SELECT to_number(SUBCONTRACT_CODE) FROM  OSP$SUBCONTRACT_AMOUNT_INFO@coeus.kuali)
)
/
...
/
CREATE TABLE TMP_SUBAWD_INF_ATT_FILE_MAPING(
SUBCONTRACT_CODE VARCHAR2(20) ,
LINE_NUMBER NUMBER(4,0) ,
FILE_ID NUMBER(22)
)
/
TRUNCATE TABLE TMP_SUBAWD_INF_ATT_FILE_MAPING
/
DROP TABLE OSP$SUBCONTRACT_AMOUNT_INFO
/
CREATE TABLE OSP$SUBCONTRACT_AMOUNT_INFO(
SUBCONTRACT_CODE VARCHAR2(20) , 
...
/
INSERT INTO OSP$SUBCONTRACT_AMOUNT_INFO(SUBCONTRACT_CODE,LINE_NUMBER,SEQUENCE_NUMBER,OBLIGATED_AMOUNT,OBLIGATED_CHANGE,ANTICIPATED_AMOUNT,
ANTICIPATED_CHANGE,EFFECTIVE_DATE,COMMENTS,UPDATE_TIMESTAMP,UPDATE_USER,FILE_NAME,DOCUMENT,MIME_TYPE,PERFORMANCE_START_DATE,
PERFORMANCE_END_DATE,MODIFICATION_NUMBER,MODIFICATION_EFFECTIVE_DATE)
SELECT SUBCONTRACT_CODE,LINE_NUMBER,SEQUENCE_NUMBER,OBLIGATED_AMOUNT,OBLIGATED_CHANGE,ANTICIPATED_AMOUNT,ANTICIPATED_CHANGE,
EFFECTIVE_DATE,COMMENTS,UPDATE_TIMESTAMP,UPDATE_USER,FILE_NAME,DOCUMENT,MIME_TYPE,PERFORMANCE_START_DATE,PERFORMANCE_END_DATE,
MODIFICATION_NUMBER,MODIFICATION_EFFECTIVE_DATE
FROM OSP$SUBCONTRACT_AMOUNT_INFO@Coeus.Kuali
/
commit
/
DECLARE
...
SELECT sa.SUBCONTRACT_CODE,sa.LINE_NUMBER,as_seq as SEQUENCE_NUMBER,sa.OBLIGATED_AMOUNT,sa.OBLIGATED_CHANGE,sa.ANTICIPATED_AMOUNT,
		sa.ANTICIPATED_CHANGE,sa.EFFECTIVE_DATE,sa.COMMENTS,sa.UPDATE_TIMESTAMP,sa.UPDATE_USER,sa.FILE_NAME,sa.DOCUMENT,sa.MIME_TYPE,
		sa.PERFORMANCE_START_DATE,sa.PERFORMANCE_END_DATE,sa.MODIFICATION_NUMBER,sa.MODIFICATION_EFFECTIVE_DATE 
FROM OSP$SUBCONTRACT_AMOUNT_INFO sa
where sa.SUBCONTRACT_CODE = as_mit
and SEQUENCE_NUMBER = ( select max(aw.sequence_number) from OSP$SUBCONTRACT_AMOUNT_INFO aw 
						where  aw.SUBCONTRACT_CODE=sa.SUBCONTRACT_CODE 
						and    aw.line_number = sa.line_number
...
EXIT WHEN c_amt_info%NOTFOUND;
	...
	IF ls_file_name IS NOT NULL THEN	
		...
		BEGIN
			...
			BEGIN
				SELECT FILE_ID into li_seq_attachment FROM TMP_SUBAWD_INF_ATT_FILE_MAPING 
				WHERE SUBCONTRACT_CODE = ls_subaward_code
				AND  LINE_NUMBER = r_amt_info.LINE_NUMBER;	
				
			EXCEPTION
			WHEN OTHERS THEN
			
				SELECT SEQ_ATTACHMENT_ID.NEXTVAL INTO li_seq_attachment FROM DUAL;				
				INSERT INTO TMP_SUBAWD_INF_ATT_FILE_MAPING(SUBCONTRACT_CODE,LINE_NUMBER,FILE_ID)
				VALUES(ls_subaward_code, r_amt_info.LINE_NUMBER, li_seq_attachment);	
...
DELETE FROM SUBAWARD_AMT_RELEASED WHERE SUBAWARD_ID IN (
	SELECT SUBAWARD_ID FROM SUBAWARD WHERE SUBAWARD_CODE in ( SELECT to_number(SUBCONTRACT_CODE) FROM  OSP$SUBCONTRACT_AMOUNT_INFO@coeus.kuali)
)
/
commit
/
