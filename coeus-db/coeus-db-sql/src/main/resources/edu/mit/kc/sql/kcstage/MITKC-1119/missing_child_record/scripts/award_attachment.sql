...
WHERE t1.MIT_AWARD_NUMBER IN(SELECT distinct MIT_AWARD_NUMBER FROM REFRESH_AWARD)
/
COMMIT
/
declare
...
REFRESH_AWARD ts on replace(a.AWARD_NUMBER,'-00','-') = ts.MIT_AWARD_NUMBER and a.SEQUENCE_NUMBER=ts.SEQUENCE_NUMBER
