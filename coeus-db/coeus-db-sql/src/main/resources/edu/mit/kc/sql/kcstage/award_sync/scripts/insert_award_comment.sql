...
declare
	li_count number;
cursor c_comment is
  select t1.MIT_AWARD_NUMBER,
  t1.SEQUENCE_NUMBER,
  t1.COMMENT_CODE,
  t1.CHECKLIST_PRINT_FLAG,
  t1.COMMENTS,
  t1.UPDATE_TIMESTAMP,
  t1.UPDATE_USER
  FROM OSP$AWARD_COMMENTS@coeus.kuali t1
...
EXIT WHEN c_awd% NOTFOUND;
 ...
 if r_awd.COMMENT_TYPE_CODE = '18' then
 ...
 elsif 	r_awd.COMMENT_TYPE_CODE = '19' then
				UPDATE AWARD_COMMENT 
