CREATE TABLE PROTO_AMEND_RENEWAL_BAK_0519(
PROTO_AMEND_RENEWAL_ID  NUMBER(12),   
PROTO_AMEND_REN_NUMBER  VARCHAR2(20) ,
DATE_CREATED            DATE,        
SUMMARY                 CLOB,         
...
/    
INSERT INTO PROTO_AMEND_RENEWAL_BAK_0519(
...
/
CREATE TABLE TMP_MISSING_AMEND_RENW_PROTO( 
PROTO_AMEND_REN_NUMBER  VARCHAR2(20) ,
PROTOCOL_NUMBER         VARCHAR2(20), 
SEQUENCE_NUMBER         NUMBER(4)
)
/
INSERT INTO TMP_MISSING_AMEND_RENW_PROTO(
PROTO_AMEND_REN_NUMBER,
PROTOCOL_NUMBER,
SEQUENCE_NUMBER
)
select s1.proto_amend_ren_number,s1.protocol_number,s1.sequence_number
from
(
select distinct t1.proto_amend_renewal_id,t1.proto_amend_ren_number,t1.protocol_number,t1.sequence_number
from proto_amend_renewal t1 
inner join protocol t2 on t1.protocol_number = t2.protocol_number 
where t2.document_number like 'MP%'
) s1 left outer join protocol s2 on s1.proto_amend_ren_number = s2.protocol_number
where s2.PROTOCOL_NUMBER is null
order by s1.protocol_number,s1.sequence_number
/
...
order by protocol_number,sequence_number;
...
exit when c_missing%notfound;
    li_seq := 0;
    select count(protocol_id) into li_count from protocol where protocol_number = r_missing.proto_amend_ren_number and sequence_number = 0; 
	...
	if li_count = 0 then
       ...
       insert into protocol(protocol_id,
					 ...
					 select SEQ_PROTOCOL_ID.NEXTVAL,
							...
							r_missing.proto_amend_ren_number,
							0,
							protocol_type_code,
							protocol_status_code,
							title,
							...
							sys_guid()
					   from protocol 
					   where protocol_number = r_missing.protocol_number
					   and sequence_number = r_missing.sequence_number;
	   exception
       when others then
       dbms_output.put_line('Exception in protocol ,for PROTO_AMEND_REN_NUMBER '||r_missing.proto_amend_ren_number||', Error is '||sqlerrm);
	   continue;
       end;	
					   
					
					begin
						insert into protocol_document(document_number,ver_nbr,update_timestamp,update_user,protocol_workflow_type,obj_id,rerouted_flag)
						select document_number,1,update_timestamp,update_user,'normal',sys_guid(),'n' from protocol
						where protocol_number = r_missing.proto_amend_ren_number
						and sequence_number = li_seq;
					exception
                    when others then
                    dbms_output.put_line('Exception in protocol_document ,for PROTO_AMEND_REN_NUMBER '||r_missing.proto_amend_ren_number||', Error is '||sqlerrm);
                    end;	
					
					
					select max(DOC_TYP_ID) into ls_doc_typ_id from KREW_DOC_TYP_T where DOC_TYP_NM='ProtocolDocument';
                    ...
                    begin
						...
						select a.document_number,ls_doc_typ_id,'F',0,sysdate,a.update_timestamp,sysdate,null,sysdate,('protocoldocument'||'-'||a.protocol_number) ttl,null,1,nvl(p.prncpl_id,'unknownuser'),1,null,null,sys_guid(),null,null 
						from protocol a left outer join krim_prncpl_t p on lower(p.prncpl_nm)=lower(a.update_user)
						where a.protocol_number = r_missing.proto_amend_ren_number
						and a.sequence_number = li_seq;
                    ...
                    begin
						...
						select a.document_number,sys_guid(),1,a.protocol_number,null,null,null from protocol a 
						where a.protocol_number = r_missing.proto_amend_ren_number
						and a.sequence_number = li_seq;
                    ...
                    begin					
						...
						select a.document_number,null from protocol a
						where a.protocol_number = r_missing.proto_amend_ren_number
						and a.sequence_number = li_seq;
					...
					begin
						...
						insert into temp_krew_2(document_number,rte_brch_id,rte_node_id,rte_node_instn_id,module)
						select document_number,krew_rte_node_s.nextval,krew_rte_node_s.nextval,krew_rte_node_s.nextval,'irb' from protocol
						where protocol_number = r_missing.proto_amend_ren_number
						and sequence_number = li_seq;
					
					exception
                    when others then
						dbms_output.put_line('Exception in temp_krew_2 ,for PROTO_AMEND_REN_NUMBER '||r_missing.proto_amend_ren_number||', Error is '||sqlerrm);
                    end;
					
					select protocol_id  into li_protocol_id from protocol where protocol_number = r_missing.proto_amend_ren_number
					and sequence_number = li_seq;
					
					
				
				    select count(submission_id) into li_cnt from protocol_submission where protocol_number = r_missing.proto_amend_ren_number and sequence_number = li_seq; 
					if li_cnt = 0 then
					...
					begin
						...
						submission_type_code,submission_type_qual_code,submission_status_code,protocol_review_type_code,submission_date,comments,yes_vote_count,no_vote_count,abstainer_count,
						voting_comments,update_timestamp,update_user,ver_nbr,obj_id,recused_count,is_billable,comm_decision_motion_type_code)
						select SEQ_PROTOCOL_ID.NEXTVAL,r_missing.proto_amend_ren_number,li_seq,submission_number,schedule_id,committee_id,li_protocol_id,schedule_id_fk,committee_id_fk,
						submission_type_code,submission_type_qual_code,submission_status_code,protocol_review_type_code,submission_date,comments,yes_vote_count,no_vote_count,abstainer_count,
						voting_comments,update_timestamp,update_user,1,sys_guid(),recused_count,is_billable,comm_decision_motion_type_code
						from protocol_submission
						where protocol_number = r_missing.protocol_number
						and sequence_number = r_missing.sequence_number;
					exception
                    when others then
                    dbms_output.put_line('Exception in protocol_submission ,for PROTO_AMEND_REN_NUMBER '||r_missing.proto_amend_ren_number||', Error is '||sqlerrm);
                    end;

					end if;
				
               
                    select count(protocol_funding_source_id) into li_cnt from protocol_funding_source where protocol_number = r_missing.proto_amend_ren_number and sequence_number = li_seq; 
					if li_cnt = 0 then				
					...
					begin
						...
						select seq_protocol_id.nextval,li_protocol_id,r_missing.proto_amend_ren_number,li_seq,funding_source_type_code,funding_source,update_timestamp,update_user,1,funding_source_name,sys_guid() 
						from protocol_funding_source
						where protocol_number = r_missing.protocol_number
						and sequence_number = r_missing.sequence_number;
					exception
                    when others then
                    dbms_output.put_line('Exception in protocol_funding_source ,for PROTO_AMEND_REN_NUMBER '||r_missing.proto_amend_ren_number||', Error is '||sqlerrm);
                    end;
					
					end if;

					
					select count(pa_protocol_id) into li_cnt from protocol_attachment_protocol where protocol_number = r_missing.proto_amend_ren_number and sequence_number = li_seq; 
					if li_cnt = 0 then	
					...
					begin
						...
						select seq_protocol_id.nextval,li_protocol_id,r_missing.proto_amend_ren_number,li_seq,type_cd,document_id,file_id,description,status_cd,contact_name,email_address,phone_number,comments,1,update_timestamp,update_user,sys_guid(),document_status_code,create_timestamp,attachment_version
						from protocol_attachment_protocol
						where protocol_number = r_missing.protocol_number
						and sequence_number = r_missing.sequence_number;
					exception
                    when others then
                    dbms_output.put_line('Exception in protocol_attachment_protocol ,for PROTO_AMEND_REN_NUMBER '||r_missing.proto_amend_ren_number||', Error is '||sqlerrm);
                    end;
					
					end if;
					
                    select count(pa_personnel_id) into li_cnt from protocol_attachment_personnel where protocol_number = r_missing.proto_amend_ren_number and sequence_number = li_seq; 
					if li_cnt = 0 then	
				    
					begin
						insert into protocol_attachment_personnel(pa_personnel_id,protocol_id_fk,protocol_number,sequence_number,type_cd,document_id,file_id,description,person_id,ver_nbr,update_timestamp,update_user,obj_id)
						select seq_protocol_id.nextval,li_protocol_id,r_missing.proto_amend_ren_number,li_seq,type_cd,document_id,file_id,description,person_id,1,update_timestamp,update_user,sys_guid()
						from protocol_attachment_personnel
						where protocol_number = r_missing.protocol_number
						and sequence_number = r_missing.sequence_number;
					exception
                    when others then
                    dbms_output.put_line('Exception in protocol_attachment_personnel ,for PROTO_AMEND_REN_NUMBER '||r_missing.proto_amend_ren_number||', Error is '||sqlerrm);
                    end;
...
select rte_brch_id,'primary',null,null,null,null,1 from temp_krew_2 where module='irb'
/ 
declare
ls_doc_typ_id varchar2(40);
begin  --umb added max() below
select max(doc_typ_id) into ls_doc_typ_id from krew_doc_typ_t where upper(doc_typ_nm) = upper('ProtocolDocument');
...
select rte_node_id,ls_doc_typ_id,'initiated','org.kuali.rice.kew.engine.node.initialnode',null,null,0,0,'p',null,1,null,null,null from temp_krew_2 where module='irb';
end;
/
...
select rte_node_instn_id,document_number,rte_node_id,rte_brch_id,null,1,0,0,1 from temp_krew_2 where module='irb'
/ 

insert into krew_init_rte_node_instn_t(doc_hdr_id,rte_node_instn_id)
select document_number,rte_node_instn_id from temp_krew_2 where module='irb'
/ 
...
select krew_actn_rqst_s.nextval,null,'c',k.doc_hdr_id,null,'a',-3,k.initr_prncpl_id,null,null,null,'u',0,null,0,t.rte_node_instn_id,null,1,sysdate,'initiator needs to complete document.',1,null,null,'f',1	,0,null,null from krew_doc_hdr_t k inner join temp_krew_2 t on k.doc_hdr_id=t.document_number where k.ttl like 'protocoldocument%' and t.module='irb' 
/ 
...
select krew_actn_tkn_s.nextval,k.doc_hdr_id,k.initr_prncpl_id,null,'s',sysdate,1,null,1,1,null from krew_doc_hdr_t k inner join temp_krew_2 t on k.doc_hdr_id=t.document_number where k.ttl like 'protocoldocument%' and t.module='irb' 
/ 
...
ps.voting_comments,ps.update_timestamp,ps.update_user,ps.recused_count,ps.is_billable,ps.comm_decision_motion_type_code from TMP_MISSING_AMEND_RENW_PROTO pa inner join PROTOCOL_SUBMISSION ps
