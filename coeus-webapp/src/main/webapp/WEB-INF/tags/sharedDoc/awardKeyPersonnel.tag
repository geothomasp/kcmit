...
<c:set var="pi" value="${KualiForm.pi}" />
		...
		<div class="tab-container" align="center">
    		...
    		<table id="contacts-table" cellpadding="0" cellspacing="0" summary="Key Personnel">
				...
				<div id="per.fullName.div">&nbsp; <c:if	test="${!empty KualiForm.projectPersonnelBean.newProjectPerson.person}">
					<c:choose>
						<c:when
				...
				<div id="rol.fullName.div">&nbsp; <c:if	test="${!empty KualiForm.projectPersonnelBean.newAwardContact}">
					<c:choose>
						<c:when test="${empty KualiForm.projectPersonnelBean.newAwardContact}">
							<span style='color: red;'>not found</span>
						</c:when>
						<c:otherwise>
