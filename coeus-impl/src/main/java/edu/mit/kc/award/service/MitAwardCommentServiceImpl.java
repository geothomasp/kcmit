 ...
 */
@Transactional
public class MitAwardCommentServiceImpl implements AwardCommentService {
    ...
    public List<CommentType> retrieveCommentTypes() {       
        List<CommentType> commentTypeListNew = new ArrayList<CommentType>();
        ...
        commentTypeListNew = sortCommentTypes(commentTypeList);        
        return commentTypeListNew;
    }
    ...
    public List<CommentType> sortCommentTypes(List<CommentType> commentTypeList) {
    	List<CommentType> commentTypeListNew = new ArrayList<CommentType>();
    	List<String> unsortedComment= new ArrayList<String>();
    	for(CommentType commentType:commentTypeList){    
            unsortedComment.add(commentType.getDescription());
        }
        java.util.Collections.sort(unsortedComment);
        int index = 0;
        int currentIndex=unsortedComment.size();      
        for(CommentType commentType:commentTypeList){
        if(index < currentIndex){    
        	for(CommentType commentTypes:commentTypeList){
        if(index < currentIndex){
            while(unsortedComment.get(index).equalsIgnoreCase(commentTypes.getDescription())){
            commentTypeListNew.add(commentTypes);
            index++;
            break;
        }}}}
        }
